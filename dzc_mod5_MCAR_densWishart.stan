...
parameters {
  ...
  matrix [N,3] phi;         // spatial effects
  cov_matrix[3] Sigma;
...
transformed parameters{
  ...
  LSigma=cholesky_decompose(Sigma)';
  for(i in 1:3)
    for(j in 1:3)
     R[i,j]=Sigma[i,j]/(sqrt(Sigma[i,i])*sqrt(Sigma[j,j]));
  for(i in 1:N){
    muS[i,]=(LSigma*phi[i,]')';
    mu[i]=exp(beta[1]+x[i,1]*beta[2]+x[i,2]*beta[3]+x[i,3]*beta[4]+muS[i,1]);
    for(c in 1:2){
      lambda[i,c]=exp(alpha[1,c]+x[i,1]*alpha[2,c]+x[i,2]*alpha[3,c]+x[i,3]*alpha[4,c]+muS[i,(c+1)]);
    }
...
model {
  ...
  /*
  for(i in 1:N){
    theta[i,]~multi_normal((muS[i]*ones)',Sigma);
  }
  */
  ...
  for(j in 1:3)
    sum(phi[,j]) ~ normal(0, 0.001 * N);  // equivalent to mean(phi) ~ normal(0,0.001)
