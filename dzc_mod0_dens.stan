...
transformed parameters{
  ...
  for(i in 1:N){
    ...
    mu[i]=exp(beta[1]+x[i,1]*beta[2]+x[i,2]*beta[3]+x[i,3]*beta[4]+muS[i]);
    for(c in 1:2){
      lambda[i,c]=exp(alpha[1,c]+x[i,1]*alpha[2,c]+x[i,2]*alpha[3,c]+x[i,3]*alpha[4,c]+muS[i]);
    }
